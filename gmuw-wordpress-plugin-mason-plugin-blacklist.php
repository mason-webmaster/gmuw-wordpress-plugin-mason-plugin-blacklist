...
// define constants
	...
	if ( ! defined( 'MAT_DISALLOWED_PLUGINS' ) ) {
		define( 'MAT_DISALLOWED_PLUGINS', [
			'elementor',
			'elementor-pro'
		] );
	}
	// define notification email. You could define a different value for this constant in the wp-config file.
	if ( ! defined( 'MAT_DISALLOWED_PLUGINS_NOTIFICATION_EMAIL' ) ) {
		define( 'MAT_DISALLOWED_PLUGINS_NOTIFICATION_EMAIL', "jmacario@gmu.edu" );
	}
...
function gmuw_plugin_api_filter( $res, $action, $arg ) {
	...
	foreach ( $res->plugins as $i => $plugin ) {
		if ( in_array( $plugin['slug'], MAT_DISALLOWED_PLUGINS ) ) {
			unset( $res->plugins[ $i ] );
		}
	}
...
function gmuw_prevent_activation( $plugin, $network_wide ) {
	...
	if ( in_array( $plug_slug[0], MAT_DISALLOWED_PLUGINS ) ) {
		throw new \Exception();
	}
}
...
function gmuw_prevent_install_plugins_cap( $allcaps, $caps, $args, $user ) {
	...
	if ( in_array( $plugin, MAT_DISALLOWED_PLUGINS ) ) {
		$allcaps['install_plugins'] = 0;

		$username  = $user->data->user_nicename;
		$useremail = $user->data->user_email;
		$site      = site_url();
		$to        = MAT_DISALLOWED_PLUGINS_NOTIFICATION_EMAIL;
